...
bool PlaneCut::CutByPlane(Plane plane) {
  ...
  *CuttedMesh = CuttedMesh->SplitModelByIsoline(scalarField, 0).first;
  ...
  Eigen::Vector3d dir1(isoline[0][1].x() - isoline[0][0].x(),
                       isoline[0][1].y() - isoline[0][0].y(),
                       isoline[0][1].z() - isoline[0][0].z()),
      dir2(vec.x(), vec.y(), vec.z());
  dir1.normalize();
  dir2.normalize();
    ...
    if (f->info().in_domain()) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        for (int j = 0; j < isoPoint.size(); j++) {
          double dis = sqrt((pt.x() - isoPoint[j].locaInPlane.x()) *
                                (pt.x() - isoPoint[j].locaInPlane.x()) +
                            (pt.y() - isoPoint[j].locaInPlane.y()) *
                                (pt.y() - isoPoint[j].locaInPlane.y()));
        ...
        for (int j = 0; j < Last_verts.size(); j++) {
          auto v = Last_verts[j];
          double dis = sqrt((v.x() - ThrDpoint.x()) * (v.x() - ThrDpoint.x()) +
                            (v.y() - ThrDpoint.y()) * (v.y() - ThrDpoint.y()) +
                            (v.z() - ThrDpoint.z()) * (v.z() - ThrDpoint.z()));
