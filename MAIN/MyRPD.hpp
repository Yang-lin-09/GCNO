...
pair<vector<vector<Eigen::Vector3d>>,
     pair<vector<double>, map<MyPoint, set<MyPoint>>>>
Comput_RPD(string modelpath, string modelName, bool IfNor) {
  ...
  // string filePath =
  // "E:\\Dropbox\\SIG-2022-Feature-preserving-recon\\data\\Result_100models\\";
  string ss;

  {
    ...
    // string filePath =
    // "D:\\SIG-BigExps\\00040123_8fc7d06caf264003a242597a_trimesh_000\\rsmall\\";
    // string modelName = "lucy";
    // string modelName = files[filenum];
    /*MyHalfEdgeModel* PoissonModel = new MyHalfEdgeModel();
    PoissonModel->ReadObjFile((filePath + "\\model_poisson_"+ modelName
    +".obj").c_str());*/
    double radis = 0;
    ...
    // PCmodel.ReadXYZFile(("data\\" + modelName +
    // "\\fandisk50000_6883.xyz").c_str(), true);
    ...
    CGAL::make_hexahedron(obb_points[0], obb_points[1], obb_points[2],
    obb_points[3], obb_points[4], obb_points[5], obb_points[6], obb_points[7],
    obb_sm);*/
    vector<Eigen::Vector3d> OBBmeshPs;
    vector<Eigen::Vector3i> OBBmeshFs;

    for (int i = 0; i < 8; i++) {
      OBBmeshPs.push_back(Eigen::Vector3d(obb_points[i].x(), obb_points[i].y(),
                                          obb_points[i].z()));
    }
    ...
    // filePath =
    // "E:\\Dropbox\\SIG-2022-Feature-preserving-recon\\data\\ToRander\\";
    for (int i = 0; i < n; i++) {
    ...
    for (const Rt::Vertex_handle vh : rt.finite_vertex_handles()) {
      ...
      for (auto nb : f_vertices) {
        if (Point2ID.find(MyPoint(nb->point().x(), nb->point().y(),
                                  nb->point().z())) == Point2ID.end()) {
          cout << "ERROR!~\n\n\n";
        }
        nb_tmps.push_back(Point2ID[MyPoint(nb->point().x(), nb->point().y(),
                                           nb->point().z())]);
        // out_P << "v " << nb->point().x() << " " << nb->point().y() << " " <<
        // nb->point().z() << "\n";
      }
      if (Point2ID.find(MyPoint(vh->point().x(), vh->point().y(),
                                vh->point().z())) == Point2ID.end()) {
        cout << "ERROR!~\n\n\n";
      }
      neighboor[Point2ID[MyPoint(vh->point().x(), vh->point().y(),
                                 vh->point().z())]] = nb_tmps;
      /*if (Point2ID.find(MyPoint(vh->point().x(), vh->point().y(),
      vh->point().z())) == Point2ID.end())
      {
    ...
    for (int ii = 0; ii < n; ii++) {
      ...
      // outPts << VersPC[ii].x() << " " << VersPC[ii].y() << " " <<
      // VersPC[ii].z() << "\n";
      ...
      for (int jj = 0; jj < neighboor[ii].size(); jj++) {
        ...
        // Eigen::Vector3d MidPoint = VersPC[ii] + (VersPC[p] -
        // VersPC[ii])*(Weight[ii]/(Weight[ii]+Weight[p]));  //voronoi
        Eigen::Vector3d MidPoint;
        // double lambda = ((r1 * r1 - r2 * r2) / ((bb - aa).norm() * (bb -
        // aa).norm()) + 1.0) / 2.0;
        MidPoint = (aa + bb) / 2.0;

        /*MidPoint.x() = (r1 * r1 - r2 * r2 + bb.x() * bb.x() - aa.x() * aa.x())
        / (2 * bb.x() - 2 * aa.x()); MidPoint.y() = (r1 * r1 - r2 * r2 + bb.y()
        * bb.y() - aa.y() * aa.y()) / (2 * bb.y() - 2 * aa.y()); MidPoint.z() =
        (r1 * r1 - r2 * r2 + bb.z() * bb.z() - aa.z() * aa.z()) / (2 * bb.z() -
        2 * aa.z());*///power
      ...
      /*string file = filePath + "\\cells\\" + modelName + "_" + to_string(ii) +
      "cells.obj"; PC->CuttedMesh->WriteObjFile(file.c_str());*/
      ...
      for (int i = 0; i < VDHPs.size(); i++) {
        ...
        for (auto f : Fs) {
          Eigen::Vector3d nor;
          nor = (VDHPs[VDHFaces[f][0]] - VDHPs[VDHFaces[f][1]])
                    .cross(VDHPs[VDHFaces[f][0]] - VDHPs[VDHFaces[f][2]]);
          nor.normalize();
          Nors.push_back(nor);
        }
        ...
        for (int i = 0; i < k - 1; i++) {
          for (int j = i + 1; j < k; j++) {
            double dij =
                min((Nors[i] - (Nors[j])).norm(), (Nors[j] - (Nors[i])).norm());
            if (dij < errorr) {
              v1[j] = v1[i];
            }
          }
...
#pragma omp parallel for
      ...
      for (int i = 0; i < VDpoints[ii].size(); i++) {
        ...
        for (int j = 0; j < n; j++) {
          wd =
              wd +
              Areas[j] *
                  (((VersPC[j] - queryP.p).dot(Normal_ori[j])) /
                   (4 * PI * pow(((VersPC[j] - queryP.p).squaredNorm()), 1.5)));
        }
        // wd =  wd;
        WindingNum[ii][i] = wd;
        if (wd > maxwd)
        ...
        if (wd < minwd)
          minwd = wd;
        // cout << "Winding Num: " << wd << " " << queryP.p.x() << " " <<
        // queryP.p.y() << " " << queryP.p.z() << endl;
      }
    }
    ...
    for (int ii = 0; ii < nnn; ii++)
    {
        ...
        {
                            Eigen::Vector3d QueryP(-0.6 + ii / 416.0, -0.6 + jj
    / 416.0, 0); double wd = 0; for (int j = 0; j < n; j++)
            {
                wd = wd + Areas[j] * (((VersPC[j] - QueryP).dot(Normal_ori[j]))
    / (4 * PI * pow(((VersPC[j] - QueryP).squaredNorm()), 1.5)));
            }
                            outWwwd << wd << " ";
        }
                    outWwwd << endl;
    ...
    for (int ii = 0; ii < n; ii++) {
      for (int i = 0; i < VDpoints[ii].size(); i++) {
        ...
        // outWd << VDpoints[ii][i].p.x() << " "  << VDpoints[ii][i].p.y() << "
        // " << VDpoints[ii][i].p.z() << " " <<
        // (WindingNum[ii][i]-minwd)/(maxwd-minwd)  << " 0 0\n";
        outWds << WindingNum[ii][i] << " " << dis << "   " << ii << " " << i
               << endl;

        if (WindingNum[ii][i] > 1.1) {
          outWd << VDpoints[ii][i].p.x() << " " << VDpoints[ii][i].p.y() << " "
                << VDpoints[ii][i].p.z() << " " << 0 << " 0 1\n";
        } else {
          if (WindingNum[ii][i] < -0.1) {
            outWd << VDpoints[ii][i].p.x() << " " << VDpoints[ii][i].p.y()
                  << " " << VDpoints[ii][i].p.z() << " " << 0 << " 1 0\n";
          } else {
            outWd << VDpoints[ii][i].p.x() << " " << VDpoints[ii][i].p.y()
                  << " " << VDpoints[ii][i].p.z() << " "
                  << (WindingNum[ii][i] - (-0.1)) / (1.1 - (-0.1)) << " 0 0\n";
          }
        }
      }
    }
