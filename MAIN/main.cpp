...
pair<double, double> V3toV2(Eigen::Vector3d nor) {
  ...
  if (u == 0) {
    ...
    double tot1, tot2;
    tot1 = (n11 - nor).x() * (n11 - nor).x() +
           (n11 - nor).y() * (n11 - nor).y() +
           (n11 - nor).z() * (n11 - nor).z();
    tot2 = (n22 - nor).x() * (n22 - nor).x() +
           (n22 - nor).y() * (n22 - nor).y() +
           (n22 - nor).z() * (n22 - nor).z();
    if (tot1 < tot2) {
      v = tmp1;
...
void WindingNumLBFGSTest(
    string modelpath, string model, vector<vector<Eigen::Vector3d>> VDPs,
    pair<vector<double>, map<MyPoint, set<MyPoint>>> areasNeibor,
    bool Ifdoublelyer) {
    ...
    for (int qpid = 0; qpid < VDPs[i].size(); qpid++) {
      ...
      if (VDPsMap.find(mp) == VDPsMap.end()) {
        ...
        for (int j = 0; j < n; j++) {
          ...
          wwd += Areas[j] *
                 ((YY.dot(Nall[j])) / (4 * PI * pow((YY.squaredNorm()), 1.5)));
        }
  ...
  cout << "Total Query points : " << Totn << " After QuChong n: " << Qcn
       << endl;
  ...
  std::function<double(const Eigen::VectorXd &X, Eigen::VectorXd &g)> fg =
      [&](const Eigen::VectorXd &X, Eigen::VectorXd &g) {
        ...
        double sigma = 0.2;
        // vector<double> WdNum;
        double lossNor = 0, lossFC = 0, losswdn = 0, loss_laplaceIn = 0,
               loss_laplaceOut = 0;
        vector<Eigen::Vector3d> Nall_new;
        ofstream outEnd;
        if (Ifdoublelyer) {
          outEnd.open(modelpath + "Out\\" + model + to_string(fgNum) +
                      "_Double_End.xyz");
        } else {
          outEnd.open(modelpath + "Out\\" + model + to_string(fgNum) +
                      "_End.xyz");
        ...
        for (int j = 0; j < n; j++) {
          ...
          outEnd << Vall[j].transpose() << " "
                 << (1.0 * Nall_new[j]).transpose() << endl;
        }
        outEnd.close();
          ...
          for (int i = 0; i < n; i++) {
            ...
            for (int qpid = 0; qpid < VDPs_QC[i].size(); qpid++) {
              ...
              if (Ifdoublelyer) {
                ...
                func = func +
                       lambda_x * (pow((wd - 0.5) / (sqrt(0.5)), 4) -
                                   pow((wd - 0.5) / (sqrt(0.5)), 2) - wd / 4);
                losswdn +=
                    lambda_x * (pow((wd - 0.5) / (sqrt(0.5)), 4) -
                                pow((wd - 0.5) / (sqrt(0.5)), 2) - wd / 4);
              } else {
...
#pragma omp parallel for // schedule(dynamic, 5)
              for (int j = 0; j < n; j++) {
                ...
                double v = X(j * 2 + 1);
                auto YY = (Vall[j] - queryP);
                // (rate*exp(rate*(move - wd)))/(exp(rate*(move - wd)) + 1)^2 -
                // (rate*exp(-rate*(wd - move + 2)))/(exp(-rate*(wd - move + 2))
                // + 1)^2
                if (Ifdoublelyer) {
                  g(j * 2) = g(j * 2) +
                             lambda_x *
                                 (4 * pow((wd - 0.5) / (sqrt(0.5)), 3) -
                                  2 * ((wd - 0.5) / (sqrt(0.5))) - 0.25) *
                                 (YY.x() * cos(v) * cos(u) +
                                  YY.y() * sin(v) * cos(u) - YY.z() * sin(u)) *
                                 (Areas[j]) /
                                 (4 * PI * pow((YY.squaredNorm()), 1.5));
                  g(j * 2 + 1) = g(j * 2 + 1) +
                                 lambda_x *
...
#pragma omp parallel for // schedule(dynamic, 5)
            ...
            for (int qpid = 0; qpid < VDPs[i].size(); qpid++) {
              ...
              g(i * 2) =
                  g(i * 2) + lambda_nor *
                                 ((Nj.x() * cos(u) * cos(v) - Nj.z() * sin(u) +
                                   Nj.y() * cos(u) * sin(v)) *
                                  wdd) /
                                 kk;
              g(i * 2 + 1) = g(i * 2 + 1) + lambda_nor *
                                                (Nj.y() * cos(v) * sin(u) -
                                                 Nj.x() * sin(u) * sin(v)) *
                                                wdd / kk;
#pragma omp parallel for // schedule(dynamic, 5)
              for (int j = 0; j < n; j++) {
                ...
                g(j * 2) =
                    g(j * 2) -
                    lambda_half * (2 * (wdd - avg)) *
                        (((YY.x() * cos(v) * cos(u) + YY.y() * sin(v) * cos(u) -
                           YY.z() * sin(u)) *
                          (Areas[j]) /
                          (4 * PI *
                           pow(((Vall[j] - queryP).squaredNorm()), 1.5))) -
                         Gavg[j * 2]) /
                        kk;
                g(j * 2 + 1) =
                    g(j * 2 + 1) -
        ...
        // if (Ifdoublelyer)
        //{
        //	outQp.open(modelpath + "Out\\" + model + to_string(fgNum) +
        //"_Double_QueryPoints.txt"); 	outwd.open(modelpath + "Out\\" + model +
        // to_string(fgNum) + "_Double_WindingNums.txt");
        // }
        // else
        //{
        ...
        //			{
        //				outQp << queryP.transpose() << " " << 0
        //<< " 1 0" << endl;
        //			}
        //			else
        //			{
        //				outQp << queryP.transpose() << " " <<
        //(WdNum[Nid.first][Nid.second] - minWD) / (maxWD - minWD) << " 0 0" <<
        // endl;
        //
        //
        //			}
        ...
        //		//outwd << WdNum[Nid.first][Nid.second] <<"    "<<i<<"
        //"<<qpid << "\n"; 		continue;
        //	}
        //
        // }
        ...
        // cout << "Func: " << func << " LossWindingNum: " << losswdn << "
        // LossNormal: " << lossNor << " LossVariance: "<<lossFC<< " NormG: "<<
        // g.norm()  << endl;
        return func;
      };
  ...
  for (int i = 0; i < n; ++i) {
    ...
    iterX(i * 2) = rand();
    iterX(i * 2 + 1) =
        rand(); // Here, we use random normal vectors to start the optimization.

    RightX(i * 2) = Q.first;
    RightX(i * 2 + 1) = Q.second;

    Eigen::Vector3d N(sin(iterX(i * 2)) * cos(iterX(i * 2 + 1)),
                      sin(iterX(i * 2)) * sin(iterX(i * 2 + 1)),
                      cos(iterX(i * 2)));
...
int main() {
  ...
  // Please set the floating point model to fast in Visual Studio to speed up
  // the code. The default number of Openmp parallel threads is 28, set
  // according to an AMD Ryzen 5950x CPU, please set different number of threads
  // according to the cpu you use to get the best running effect.
  //
  // In order to allow you to view the optimization process in more detail,
  // we have not set the optimization stop condition, you can manually stop the
  // optimization, and view all iteration results in the data\out folder

  // A noise-free point cloud generally requires about 50 iterations, and a
  // noisy point cloud may require more.
  ...
  omp_set_num_threads(12);
  {
    ...
    cout << modelname << endl;
    bool Ifdoublelyer =
        1; // change this to 0 if you want to try f_{01} = \sum_j^M -w_j

    auto VDPs = Comput_RPD(
        modelpath, modelname,
        1); // Note that this is not a parallel version of Voronoi diagram
            // computation. We will release the parallel version later.
    cout << "Compute 3D Voronoi DONE>> \n";

    std::cout << "====================WindingNumLBFGSTest" << std::endl;
    WindingNumLBFGSTest(modelpath, modelname, VDPs.first, VDPs.second,
                        Ifdoublelyer);
