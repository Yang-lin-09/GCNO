...
public:
  MyPointCloudModel() {}
  MyPointCloudModel(vector<Eigen::Vector3d> verts,
...
void MyPointCloudModel::ReadXYZFile(const char *filename, bool WithNor) {
  ...
  if (WithNor) {
    ...
    while (in >> Point.x() >> Point.y() >> Point.z() >> Normal.x() >>
           Normal.y() >> Normal.z()) {
...
void MyPointCloudModel::WriteXYZFile(const char *filename, bool WithNor) const {
  ...
  if (WithNor) {
    int n = m_verts.size();
    for (int i = 0; i < n; i++) {
      out << setiosflags(ios::fixed) << setprecision(9)
          << m_verts[i].transpose() << " " << m_normals[i].transpose() << endl;
    }
  } else {
    int n = m_verts.size();
    for (int i = 0; i < n; i++) {
      out << setiosflags(ios::fixed) << setprecision(9)
          << m_verts[i].transpose() << endl;
    }
  }
  out.close();
}
