...
void Comput_rnn(string modelpath, string modelname) {
...

  {
    ...
    PoissonModel->ReadObjFile(
        (file + "model_poisson_" + modelname + ".obj").c_str());

    /*ofstream outOFF(file + "model_poisson.off");
    outOFF << "OFF\n";
    outOFF << PoissonModel->GetVertices().size() << " " <<
    PoissonModel->GetFaces().size() << " 0\n"; for (auto p :
    PoissonModel->GetVertices())
    {
            outOFF << p.x() << " " << p.y() << " " << p.z() << "\n";
    }
    ...
    for (int i = 0; i < n; i++) {
      ...
      // cout << "radiusSearch(): radius=" << search_radius << " -> " <<
      // nMatches << " matches\n"; outknn << nMatches << " ";
      ...
      // "]=" << ret_matches[i].second << endl; outknn << "\n";
    }
      ...
      if (mp.second.size() > 0) {
        ...
        for (int i = 0; i < pts.size(); i++) {
          ...
          for (int j = 0; j < pts.size(); j++) {
            dis += (VersPC_ori[pts[i]] - VersPC_ori[pts[j]]).norm() +
                   (VersPC[pts[i]] - VersPC[pts[j]]).norm();
          }
          if (dis < mndis) {
            mndis = dis;
            tp = pts[i];
    ...
    /*for (int ii = 0; ii < N; ii++)
    {
            ...
            outNpsori << VersPC_ori[ii].transpose() << " " <<
    Nors[ii].transpose() << endl;
    ...
    for (int ii = 0; ii < MargedPoints.size(); ii++) {
      ...
      /*if (isFeature[ii] == 1)
      {
              ...
              outNpsori << MargedPoints_Ori[ii].transpose() << " 1.0 0.1 0.1" <<
      endl;
      }
      else
      {
              outNps << MargedPoints[ii].transpose() << " 0.0 0.1 0.1" << endl;
              outNpsori << MargedPoints_Ori[ii].transpose() << " 0.0 0.1 0.1" <<
      endl;
      }*/

      outNps << MargedPoints[ii].transpose() << endl;
      outNpsori << MargedPoints_Ori[ii].transpose() << " "
                << Nors[isFeature[ii]].transpose() << endl;
    ...
    my_kd_tree_t index2(3 /*dim*/, cloud,
                        KDTreeSingleIndexAdaptorParams(10 /* max leaf */));
    index2.buildIndex();
    n = MargedPoints.size();
    int maxk = -1, mink = 999;
    if (0)
      for (int i = 0; i < n; i++) {
        ...
        double query_pt[3] = {MargedPoints[i].x(), MargedPoints[i].y(),
                              MargedPoints[i].z()};
        ...
        const size_t nMatches = index2.radiusSearch(&query_pt[0], search_radius,
                                                    ret_matches, params);
    ...
    maxk = -1;
    mink = 999;
    my_kd_tree_t index3(3 /*dim*/, cloud,
                        KDTreeSingleIndexAdaptorParams(10 /* max leaf */));
    index3.buildIndex();
    n = MargedPoints.size();
    for (int i = 0; i < n; i++) {
      ...
      const double search_radius =
          static_cast<double>((radis * 1.4) * (radis * 1.4));
      ...
      size_t nMatches =
          index3.radiusSearch(&query_pt[0], search_radius, ret_matches, params);
