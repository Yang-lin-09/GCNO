...
def realTests(window, nanoflannFlag, flannFlag, fastannFlag, libkdtreeFlag):
    ...
    os.system('realTests/./realTests.py ' + filename1 + ' ' + filename2 + ' ' +
              str(libState[0]) + ' ' + str(libState[1]) + ' ' +
              str(libState[2]) + ' ' + str(libState[3]))
    window.destroy()
    return
...
def selectFile(idx, btn):
    ...
    dir_path = os.path.dirname(
        os.path.realpath(__file__)) + '/realTests/dat_avz'
    if (idx == 0):
...
def getParamsReal():
    ...
    btn1 = tk.Button(window,
                     ...
                     command=lambda: selectFile(0, btn1),
                     width=80,
                     height=3,
                     pady=5)
    ...
    btn2 = tk.Button(window,
                     ...
                     command=lambda: selectFile(1, btn2),
                     width=80,
                     height=3,
                     pady=5)
    ...
    C1 = tk.Checkbutton(window,
                        text="nanoflann",
                        variable=nanoflannFlag,
                        onvalue=1,
                        command=lambda: DarkenLabel(C1),
                        background='white',
                        offvalue=0,
                        height=5,
                        width=20)
    ...
    C3 = tk.Checkbutton(window,
                        text="fastann",
                        variable=fastannFlag,
                        onvalue=1,
                        command=lambda: DarkenLabel(C3),
                        background='white',
                        offvalue=0,
                        height=5,
                        width=20)
    ...
    C4 = tk.Checkbutton(window,
                        text="libkdtree",
                        variable=libkdtreeFlag,
                        onvalue=1,
                        command=lambda: DarkenLabel(C4),
                        background='white',
                        offvalue=0,
                        height=5,
                        width=20)
    ...
    btn3 = tk.Button(
        window,
        text='submit',
        command=lambda: realTests(window, nanoflannFlag, flannFlag,
                                  fastannFlag, libkdtreeFlag))
    btn3.grid(row=3, column=0, pady=10)

    return


def randomTests(window, numPoints, nanoflannFlag, flannFlag, fastannFlag,
                libkdtreeFlag):
    libState = [
        nanoflannFlag.get(),
        flannFlag.get(),
    ...
    os.system('randomTests/./randomTests.py ' + str(numPoints) + ' ' +
              str(libState[0]) + ' ' + str(libState[1]) + ' ' +
              str(libState[2]) + ' ' + str(libState[3]))
    # close window
    window.destroy()
...
def getParams():
    ...
    btn = tk.Button(
        window,
        text='submit',
        command=lambda: randomTests(window, numPoints, nanoflannFlag,
                                    flannFlag, fastannFlag, libkdtreeFlag))
...
def createButton(TextMessage, row_index, col_index):
    ...
    if (TextMessage == 'Real Test'):
        btn = tk.Button(
            ...
            font=helv36,
            text=TextMessage,
            command=getParamsReal,
            foreground='black',
            background='white',
            activeforeground='white',
            activebackground='green')  #create a button inside frame
...
if __name__ == '__main__':
    ...
    realLabel = tk.Label(
        root,
        text=
        "Real Test: Compare Nanoflann performance with other kd-tree libraries on real dataset.\n Run Real Test and select two files of same object with different pose.\n",
        height=5)
    randomLabel = tk.Label(
        root,
        text=
        "Random Test: Compare Nanoflann performance with other kd-tree libraries on random dataset.\n Run Random Test and select parameters to run the test.\n",
        height=5)
    # 2 buttons
