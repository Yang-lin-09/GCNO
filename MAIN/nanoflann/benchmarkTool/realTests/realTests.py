...
def cal(MatrixTime, col):
    ...
    for i in range(numRepetitions):
        stdVal = stdVal + (MatrixTime[i][col][0] -
                           meanVal) * (MatrixTime[i][col][0] - meanVal)
...
def plotTime(execPath, numRepetitions, numDivisions):
    BuildTime = [[0.0 for x in range(numDivisions)]
                 for y in range(numRepetitions)]
    QueryTime = [[0.0 for x in range(numDivisions)]
                 for y in range(numRepetitions)]
    xaxis = []
    ...
    for processCount in range(numRepetitions):
        ...
        for it, item in enumerate(List):
            if (it < numDivisions):
                ...
                BuildTime[processCount][it -
                                        numDivisions] = [float(item) * 1000]
            else:
                QueryTime[processCount][it - 2 * numDivisions] = [
                    float(item) * 1000
                ]
    BuildTimeFinal = []
    BuildTimeError = []
...
if __name__ == '__main__':
    ...
    if (len(sys.argv) != 7 or os.path.exists(sys.argv[1]) == False
    ...
    fig, ax = plt.subplots()
    if (nanoflannFlag):
        xaxis, nanoflannBuildTimeFinal, nanoflannBuildTimeError, nanoflannQueryTimeFinal, nanoflannQueryTimeError = plotTime(
            dir_path + './benchmark_nanoflann_real' + ' ' + sys.argv[1] + ' ' +
            sys.argv[2], numRepetitions, numDivisions)
        plt.plot(xaxis,
        ...
        plt.errorbar(xaxis,
                     nanoflannBuildTimeFinal,
                     color='k',
                     yerr=nanoflannBuildTimeError,
                     fmt='o')
    if (flannFlag):
        xaxis, flannBuildTimeFinal, flannBuildTimeError, flannQueryTimeFinal, flannQueryTimeError = plotTime(
            dir_path + './benchmark_flann_real' + ' ' + sys.argv[1] + ' ' +
            sys.argv[2], numRepetitions, numDivisions)
        plt.plot(xaxis, flannBuildTimeFinal, 'g', label='flann', linewidth=3.0)
        plt.errorbar(xaxis,
                     flannBuildTimeFinal,
                     color='k',
                     yerr=flannBuildTimeError,
                     fmt='o')
    if (fastannFlag):
        xaxis, fastannBuildTimeFinal, fastannBuildTimeError, fastannQueryTimeFinal, fastannQueryTimeError = plotTime(
            dir_path + './benchmark_fastann_real' + ' ' + sys.argv[1] + ' ' +
            sys.argv[2], numRepetitions, numDivisions)
        plt.plot(xaxis,
        ...
        plt.errorbar(xaxis,
                     fastannBuildTimeFinal,
                     color='k',
                     yerr=fastannBuildTimeError,
                     fmt='o')
    if (libkdtreeFlag):
        xaxis, libkdtreeBuildTimeFinal, libkdtreeBuildTimeError, libkdtreeQueryTimeFinal, libkdtreeQueryTimeError = plotTime(
            dir_path + './benchmark_libkdtree_real' + ' ' + sys.argv[1] + ' ' +
            sys.argv[2], numRepetitions, numDivisions)
        plt.plot(xaxis,
