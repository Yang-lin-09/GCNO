...
% Cell array of colros.
    ...
    figure(1);
    for
      i = 1 : length(MAXs), MaxLeaf = MAXs(i);
    idxs = find(D( :, 2) == MaxLeaf);

    TBs = D(idxs, 3);
    % Time of tree Builds TQs = D(idxs, 4);
    % Time of tree Queries
    ...
    % Plot a 95 %
        ellipse : my_plot_ellipse(
                      [TBm TQm], 2 * [TBstd TQstd],
                      COLs { 1 + mod(i - 1, length(COLs)) });

    % plot(1e3 * TBs, 1e6 * TQs, '.', 'color',
           COLs{1 + mod(i - 1, length(COLs))});
    ...
    title(sprintf('nanoflann performance vs. Leaf Max.Size (#points=%.01e)',
                  D(1, 1)));

    end

        function[] = my_plot_ellipse(Ms, STDs, COL) angs =
            linspace(0, 2 * pi, 200);
