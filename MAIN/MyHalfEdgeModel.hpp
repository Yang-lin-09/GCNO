...
public:
  ...
  MyHalfEdgeModel() {}
  MyHalfEdgeModel(vector<Eigen::Vector3d> verts, vector<Eigen::Vector3i> faces)
      : MyBaseModel(verts, faces) {}
  vector<int> GetFacesByPoint(int id) { return fromVert2Face[id]; }
  vector<MyMeshEdge> GetEdges() { return m_edges; }
...
void MyHalfEdgeModel::CreateMeshEdges() {
  ...
  for (int i = 0; i < m_faces.size(); ++i) {
    ...
    for (int j = 0; j < 3; j++) {
      ...
      fromEdge2ID[make_pair(m_edges[3 * i + j].leftVert,
                            m_edges[3 * i + j].rightVert)] = 3 * i + j;
      // 求边长
      m_edges[3 * i + j].length = (m_verts[m_edges[3 * i + j].leftVert] -
                                   m_verts[m_edges[3 * i + j].rightVert])
                                      .norm();

      Eigen::Vector3d v1, v2;
      v1 = m_verts[m_edges[3 * i + j].indexOfOppositeVertex] -
           m_verts[m_edges[3 * i + j].leftVert];
      v2 = m_verts[m_edges[3 * i + j].rightVert] -
           m_verts[m_edges[3 * i + j].leftVert];
      ...
      v1 = m_verts[m_edges[3 * i + j].indexOfOppositeVertex] -
           m_verts[m_edges[3 * i + j].rightVert];
      v2 = m_verts[m_edges[3 * i + j].leftVert] -
           m_verts[m_edges[3 * i + j].rightVert];
  ...
  for (int i = 0; i < m_edges.size(); i++) {
    if (fromEdge2ID.find(make_pair(m_edges[i].rightVert,
                                   m_edges[i].leftVert)) != fromEdge2ID.end()) {
      m_edges[i].indexOfReverseEdge =
          fromEdge2ID[make_pair(m_edges[i].rightVert, m_edges[i].leftVert)];
    } else {
...
int MyHalfEdgeModel::GetNumberOfComponents() {
  ...
  while (!unVisistedVertices.empty()) {
    ...
    while (!tobedeleted.empty()) {
      ...
      tobedeleted.pop();
      auto firstEdge =
          fromVert2Edge[top]; //@chengjialong
                              // 找到的边为f中最后一条以top为起点的边

      auto nxtEdge = firstEdge; //
      do {

        if (unVisistedVertices.find(m_edges[nxtEdge].rightVert) !=
            unVisistedVertices.end()) // 若由v2e找到的边的右端点未被访问过
        {
          ...
          nxtEdge = m_edges[m_edges[nxtEdge].indexOfPreviousEdge]
                        .indexOfReverseEdge; // 找到这条边的前边的反边
