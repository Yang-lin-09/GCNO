...
public:
  ...
  ExtractIsoline(const vector<double> &scalarField, double val) const;
  void SaveIsoline(const char *filename,
                   const vector<vector<Eigen::Vector3d>> &) const;
  pair<MyBaseModel, MyBaseModel>
  SplitModelByIsoline(const vector<double> &scalarField, double val) const;
  void WriteTexturedObjFile(const char *filename,
                            const vector<pair<double, double>> &uvs) const;
  void WriteTexturedObjFile(const char *filename,
                            const vector<double> &uvs) const;
  ...
  while (in.getline(buf, sizeof buf)) {
    ...
    if (word == "v") {
      ...
      for (int i = 1; i <= indices.size() - 2; ++i) {
        m_faces.push_back(Eigen::Vector3i(indices[0] - 1, indices[i] - 1,
                                          indices[i + 1] - 1));
      }
    }
  }
...
void MyBaseModel::WriteObjFile(const char *filename) const {
  ...
  for (auto v : m_verts) {
    out << "v " << setiosflags(ios::fixed) << setprecision(9) << v.transpose()
        << endl;
  }
...
vector<vector<Eigen::Vector3d>>
MyBaseModel::ExtractIsoline(const vector<double> &scalarField,
                            double val) const {
  ...
  for (auto t : m_faces) {
    ...
    if ((scalarField[id1] > val) != (scalarField[id2] > val)) {
      ...
      if (pos == pointsOnEdges.end()) {
        double proportion =
            (scalarField[id2] - val) / (scalarField[id2] - scalarField[id1]);
    ...
    if ((scalarField[id2] > val) != (scalarField[id3] > val)) {
      ...
      if (pos == pointsOnEdges.end()) {
        double proportion =
            (scalarField[id3] - val) / (scalarField[id3] - scalarField[id2]);
    ...
    if ((scalarField[id3] > val) != (scalarField[id1] > val)) {
      ...
      if (pos == pointsOnEdges.end()) {
        double proportion =
            (scalarField[id1] - val) / (scalarField[id1] - scalarField[id3]);
...
void MyBaseModel::SaveIsoline(
    const char *filename,
    const vector<vector<Eigen::Vector3d>> &isoline) const {
  ofstream out(filename);
  out << "g 3d_lines" << endl;
  int cnt(0);
...
vector<Eigen::Vector3d> MyBaseModel::GetVertices() const { return m_verts; }

vector<Eigen::Vector3i> MyBaseModel::GetFaces() const { return m_faces; }

pair<MyBaseModel, MyBaseModel>
MyBaseModel::SplitModelByIsoline(const vector<double> &scalarField,
                                 double val) const {
  ...
  for (int index = 0; index < m_faces.size(); ++index) {
    ...
    if ((scalarField[id1] > val) != (scalarField[id2] > val)) {
      ...
      double proportion =
          (scalarField[id2] - val) / (scalarField[id2] - scalarField[id1]);
      Eigen::Vector3d intersection = proportion * v1 + (1 - proportion) * v2;
      if (fromTwoIDs2IDofNewVert.find(make_pair(
              min(id1, id2), max(id1, id2))) == fromTwoIDs2IDofNewVert.end()) {
        verts_copy.push_back(intersection); // verts_copy.size() - 1;
        fromTwoIDs2IDofNewVert[make_pair(min(id1, id2), max(id1, id2))] =
            verts_copy.size() - 1;
      } else {
      }
    ...
    if ((scalarField[id2] > val) != (scalarField[id3] > val)) {
      ...
      Eigen::Vector3d intersection = proportion * v2 + (1 - proportion) * v3;
      if (fromTwoIDs2IDofNewVert.find(make_pair(
              min(id2, id3), max(id2, id3))) == fromTwoIDs2IDofNewVert.end()) {
        verts_copy.push_back(intersection); // verts_copy.size() - 1;
        fromTwoIDs2IDofNewVert[make_pair(min(id2, id3), max(id2, id3))] =
            verts_copy.size() - 1;
      } else {
      }
    }
    if ((scalarField[id3] > val) != (scalarField[id1] > val)) {
      ...
      Eigen::Vector3d intersection = proportion * v3 + (1 - proportion) * v1;
      if (fromTwoIDs2IDofNewVert.find(make_pair(
              min(id3, id1), max(id3, id1))) == fromTwoIDs2IDofNewVert.end()) {
        verts_copy.push_back(intersection); // verts_copy.size() - 1;
        fromTwoIDs2IDofNewVert[make_pair(min(id3, id1), max(id3, id1))] =
            verts_copy.size() - 1;
      } else {
      }
    }
  ...
  for (int index = 0; index < m_faces.size(); ++index) {
    ...
    double average =
        1.0 / 3.0 *
        (scalarField[m_faces[index].x()] + scalarField[m_faces[index].y()] +
         scalarField[m_faces[index].z()]);
    if (average > val) {
  ...
  for (int index = 0; index < facesInLess.size(); ++index) {
    ...
    facesInLess[index] =
        Eigen::Vector3i(fromOldid2Newid[ids[0]], fromOldid2Newid[ids[1]],
...
void MyBaseModel::WriteTexturedObjFile(
    const char *filename, const vector<pair<double, double>> &uvs) const {
  ofstream out(filename);
  out << "# " << m_verts.size() << " vertices " << endl;
  out << "# " << m_faces.size() << " faces " << endl;
  ...
  for (auto uv : uvs) {
    out << "vt " << setiosflags(ios::fixed) << setprecision(9) << uv.first
        << " " << uv.second << endl;
  }

  for (auto f : m_faces) {
    auto ids = (f + Eigen::Vector3i(1, 1, 1)).transpose();
    out << "f " << ids.x() << "/" << ids.x() << " " << ids.y() << "/" << ids.y()
        << " " << ids.z() << "/" << ids.z() << endl;
  }
...
void MyBaseModel::WriteTexturedObjFile(const char *filename,
                                       const vector<double> &uvs) const {
  ofstream out(filename);
  out << "# " << m_verts.size() << " vertices " << endl;
  out << "# " << m_faces.size() << " faces " << endl;
  ...
  for (auto uv : uvs) {
    out << "vt " << setiosflags(ios::fixed) << setprecision(9) << uv << " " << 0
        << endl;
  }
