...
_CVT3D::_CVT3D(const _ManifoldModel &model,
    ...
    : _model(model), _RVD(model), _rho(rho), _para(para) {}
_CVT3D::_CVT3D(const _ManifoldModel &model,
               std::function<double(_Point3 &p)> &rho, _LBFGS::_Parameter para,
...
void _CVT3D::calculate_(int num_sites) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    _sites[i] = _model.face_(fid).point(0) * l0 +
                _model.face_(fid).point(1) * l1 +
                _model.face_(fid).point(2) * l2;
  }
        ...
        for (int i = 0; i < num; ++i) {
          for (int j = 0; j < cells[i].size(); ++j) {
            Eigen::VectorXd inte = BGAL::_Integral::integral_triangle3D(
                ...
                _RVD.vertex_(std::get<2>(cells[i][j])));
            energy += inte(1);
            g(i * 3) += inte(2);
            g(i * 3 + 1) += inte(3);
            g(i * 3 + 2) += inte(4);
...
void _CVT3D::calculate_CapVT(std::vector<BGAL::_Point3> &sites) {
  ...
  for (int i = 0; i < cells.size(); ++i) {
    for (int j = 0; j < cells[i].size(); ++j) {
      ...
      side[0] = sqrt(pow(a.x() - b.x(), 2) + pow(a.y() - b.y(), 2) +
                     pow(a.z() - b.z(), 2));
      side[1] = sqrt(pow(a.x() - c.x(), 2) + pow(a.y() - c.y(), 2) +
                     pow(a.z() - c.z(), 2));
      side[2] = sqrt(pow(c.x() - b.x(), 2) + pow(c.y() - b.y(), 2) +
                     pow(c.z() - b.z(), 2));
      ...
      [&](const Eigen::VectorXd &X, Eigen::VectorXd &g) {
        ...
        for (int i = 0; i < cells.size(); ++i) {
          ...
          for (int j = 0; j < cells[i].size(); ++j) {
            ...
            double p = (side[0] + side[1] + side[2]) / 2;
            double area =
                sqrt(p * (p - side[0]) * (p - side[1]) * (p - side[2]));
            CellsArea += area;
          }
        ...
        for (int i = 0; i < num; i++) {
          ...
          for (auto ee : Edges[i]) {
            ...
            double tmp = (CellAreas[i] - CellAreas[j]) /
                         (_sites[j] - _sites[i]).length_();
        ...
        if (projection) {
          ...
          for (int i = 0; i < num; i++) {
            ...
            Plane pl(f->halfedge()->vertex()->point(),
  ...
  for (int i = 0; i < num; ++i) {
    ...
    // BGAL::_Point3 p(X(i * 3), X(i * 3 + 1), X(i * 3 + 2));
    _sites[i] = p;
    //_sites[i] = BGAL::_Point3(iterX(i * 3), iterX(i * 3 + 1), iterX(i * 3 +
    // 2));
  }
  _RVD.calculate_(_sites);
...
void _CVT3D::calculate_CapCVTByGD(std::vector<BGAL::_Point3> &sites) {
  ...
  while (1) {
    ...
    for (int i = 0; i < num; ++i) {
      ...
      _sites[i] =
          BGAL::_Point3(iterX(i * 3), iterX(i * 3 + 1), iterX(i * 3 + 2));
    }
