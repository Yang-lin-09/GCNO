...
namespace BGAL {
_CPD3D::_CPD3D(const _ManifoldModel &model)
    : _model(model), _RPD(model), _para() {
...
_CPD3D::_CPD3D(const _ManifoldModel &model,
    ...
    : _model(model), _RPD(model), _rho(rho), _para(para) {
  _max_count = 50;
  _omt_eps = 1e-4;
  _pinvtoler = 1e-6;
  _hessian_eps = 1e-10;
...
void _CPD3D::calculate_(const std::vector<double> &capacity,
  ...
  for (int i = 0; i < num; ++i)
  {
          ...
          l1 /= sum;
          l2 /= sum;
          _sites[i] = _model.face_(fid).point(0) * l0 +
  _model.face_(fid).point(1) * l1 + _model.face_(fid).point(2) * l2;
  }*/
  _sites = sites;
        ...
        for (int i = 0; i < num; ++i) {
          ...
          for (auto &kv : edges[i]) {
            ...
            for (auto &te : kv.second) {
              len +=
                  (_RPD.vertex_(te.first) - _RPD.vertex_(te.second)).length_();
            ...
            double hij = -_rho(mid_p) * (len)*0.5 /
                         ((_sites[i] - _sites[kv.first]).length_());
  ...
  std::function<bool(const Eigen::VectorXd &x, double &e, Eigen::VectorXd &g)>
      omt_fg = [&](const Eigen::VectorXd &x, double &e, Eigen::VectorXd &g) {
        e = 0;
        g.setZero();
        for (int i = 0; i < num; ++i) {
          _weights[i] = x(i);
        ...
        for (int i = 0; i < num; ++i) {
          for (int j = 0; j < cells[i].size(); ++j) {
            Eigen::VectorXd inte = BGAL::_Integral::integral_triangle3D(
                ...
                _RPD.vertex_(std::get<0>(cells[i][j])),
                _RPD.vertex_(std::get<1>(cells[i][j])),
                _RPD.vertex_(std::get<2>(cells[i][j])));
