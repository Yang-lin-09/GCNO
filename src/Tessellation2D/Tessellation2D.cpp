...
// std::vector<BKHao::_Polygon> BKHao::_Tessellation2D::get_cell_polygons_()
// const
//{
...
_Tessellation2D::get_cells_() const {
  return _cells;
}
...
_BOC::_Sign
_Tessellation2D::symbolic_point_site_(const int &site1, const int &site2,
                                      const std::pair<int, int> &symp,
                                      const int &cur_edge) {
  _BOC::_Sign r;
  // std::cout << site1 << " " << site2 << " " << symp.first << "  " <<
  // symp.second << std::endl;
  if (symp.first < 0 && symp.second < 0) {
    if (symp.first - symp.second == -1)
      r = _Side2D::side1_(_sites[site1].x(), _sites[site1].y(), _weights[site1],
                          _sites[site2].x(), _sites[site2].y(), _weights[site2],
      ...
      r = _Side2D::side1_(_sites[site1].x(), _sites[site1].y(), _weights[site1],
                          _sites[site2].x(), _sites[site2].y(), _weights[site2],
                          _boundary[0].x(), _boundary[0].y());
  } else {
    int site3 = symp.first > 0 ? symp.first - 1 : symp.second - 1;
    r = _Side2D::side2_(_sites[site1].x(), _sites[site1].y(), _weights[site1],
                        _sites[site2].x(), _sites[site2].y(), _weights[site2],
                        _sites[site3].x(), _sites[site3].y(), _weights[site3],
                        _boundary[cur_edge].x(), _boundary[cur_edge].y(),
...
_Point2 _Tessellation2D::convert_sym_to_point_(const std::set<int> &sym) {
  ...
  if (e.size() == 0) {
    ...
    double d0 = -0.5 * (v0.dot_((_sites[s[1]] + _sites[s[0]])) +
                        (_weights[s[0]] - _weights[s[1]]));
    double d1 = -0.5 * (v1.dot_((_sites[s[2]] + _sites[s[0]])) +
                        (_weights[s[0]] - _weights[s[2]]));
...
void _Tessellation2D::calculate_() {
  ...
  for (int i = 0; i < _num_sites; ++i) {
    points.push_back(std::make_pair(
        Wpoint(Point(_sites[i].x(), _sites[i].y()), _weights[i]), i));
  }
  ...
  std::vector<std::map<std::pair<int, int>, std::pair<int, int>>>
      boundary_edges(_num_sites);
  std::vector<std::map<std::pair<int, int>, std::pair<int, int>>>
      tessellation_edges(_num_sites);
  std::vector<std::vector<std::pair<std::pair<int, int>, int>>> boundary_cross(
      _num_sites);

  const _Point2 edge_center = (_boundary[0] + _boundary[1]) * 0.5;
  int nearest_site =
      rt.nearest_power_vertex(Point(edge_center.x(), edge_center.y()))->info();
  ...
  while (!Qse.empty()) {
    ...
    Qse.pop();
    std::pair<int, int> sym_ps = std::make_pair(
        -(cur_edge + _boundary.num_() - 1) % (_boundary.num_()) - 1,
        -cur_edge - 1);
    std::pair<int, int> sym_pt =
        std::make_pair(-cur_edge - 1, -(cur_edge + 1) % (_boundary.num_()) - 1);
    bool s_degraded = false;
    int s_degrad_site = -1;
    bool t_degraded = false;
    int t_degrad_site = -1;
    for (auto it = _adjacent_edges[cur_site].begin();
         it != _adjacent_edges[cur_site].end(); ++it) {
      ...
      } else if (side_s == _BOC::_Sign::PositivE &&
                 side_t == _BOC::_Sign::NegativE) {
        t_degraded = false;
        sym_pt = std::make_pair(-cur_edge - 1, it->first + 1);
      }
      // else if ((side_s == _BOC::_Sign::NegativE && side_t ==
      // _BOC::_Sign::ZerO)
      //	|| (side_s == _BOC::_Sign::ZerO && side_t ==
      //_BOC::_Sign::NegativE))
      //{
      ...
      // }
      ...
      } else if (side_s == _BOC::_Sign::ZerO &&
                 side_t == _BOC::_Sign::NegativE) {
        ...
        if ((_boundary[(cur_edge + 1) % (_boundary.num_())] -
             _boundary[cur_edge])
                .cross_(_sites[cur_site] - _sites[it->first])
                .z() < 0) {
          sym_ps = std::make_pair(0, 0);
          sym_pt = std::make_pair(0, 0);
          break;
        }
      } else if (side_s == _BOC::_Sign::NegativE &&
                 side_t == _BOC::_Sign::NegativE) {
        s_degraded = t_degraded = false;
        sym_ps = std::make_pair(0, 0);
        sym_pt = std::make_pair(0, 0);
    ...
    if (sym_ps.first == 0) {
      if (sym_pt.first != 0) {
        ...
        if (site_edge_pair_is_visited.find(std::make_pair(site3, cur_edge)) ==
            site_edge_pair_is_visited.end()) {
      ...
      if (sym_ps.first < 0 && sym_ps.second < 0) {
        if (site_edge_pair_is_visited.find(
                std::make_pair(cur_site, (cur_edge + _boundary.num_() - 1) %
                                             (_boundary.num_()))) ==
            site_edge_pair_is_visited.end()) {
          site_edge_pair_is_visited.insert(
              std::make_pair(cur_site, (cur_edge + _boundary.num_() - 1) %
                                           (_boundary.num_())));
          Qse.push(std::make_pair(cur_site, (cur_edge + _boundary.num_() - 1) %
                                                (_boundary.num_())));
        }
      } else {
      ...
      if (sym_pt.first < 0 && sym_pt.second < 0) {
        if (site_edge_pair_is_visited.find(std::make_pair(
                cur_site, (cur_edge + 1) % (_boundary.num_()))) ==
            site_edge_pair_is_visited.end()) {
          site_edge_pair_is_visited.insert(
              std::make_pair(cur_site, (cur_edge + 1) % (_boundary.num_())));
          Qse.push(
              std::make_pair(cur_site, (cur_edge + 1) % (_boundary.num_())));
        }
      } else {
  ...
  for (int i = 0; i < _num_sites; ++i) {
    if (boundary_cross[i].size() > 0) {
      for (int j = 0; j < boundary_cross[i].size(); ++j) {
        if (boundary_cross[i][j].second == 0 ||
            boundary_cross[i][j].second == -2) {
          if (boundary_cross[i][j].second == -2) {
            int __bv = -boundary_cross[i][j].first.second - 1;
            std::pair<int, int> __new_v = std::make_pair(
                -(__bv + _boundary.num_() - 1) % (_boundary.num_()) - 1,
                -__bv - 1);
  ...
  std::vector<std::map<std::pair<int, int>, int>> from_edge_start_to_adjacent(
      _num_sites);
  for (int i = 0; i < _num_sites; ++i) {
    tessellation_edges[i] = boundary_edges[i];
    for (auto it = boundary_edges[i].begin(); it != boundary_edges[i].end();
         ++it) {
    ...
    do {
      if (next_vertex.first < 0 && next_vertex.second < 0) {
        ...
        for (int i = 0; i < boundary_cross[cur_site].size(); ++i) {
          if ((boundary_cross[cur_site][i].second == -2) &&
              (boundary_cross[cur_site][i].first.second == __vid)) {
        ...
        adj_site =
            next_vertex.first > 0 ? next_vertex.first : next_vertex.second;
      } else {
      ...
      for (int i = 0; i < boundary_cross[cur_site].size(); ++i) {
        if ((boundary_cross[cur_site][i].second == 1 ||
             boundary_cross[cur_site][i].second == -1) &&
            (boundary_cross[cur_site][i].first.first == adj_site ||
             boundary_cross[cur_site][i].first.second == adj_site)) {
          if (boundary_cross[cur_site][i].second == -1) {
            ...
            tessellation_edges[cur_site][next_vertex] =
                boundary_cross[cur_site][i].first;
            from_edge_start_to_adjacent[cur_site][next_vertex] = (adj_site - 1);
            next_vertex = boundary_cross[cur_site][i].first;
          }
          while (boundary_edges[cur_site].find(next_vertex) !=
                 boundary_edges[cur_site].end()) {
      ...
      if (!is_done) {
        ...
        if (!site_visited[next_site]) {
          ...
          Qss.push(std::make_pair(next_site,
                                  std::make_pair(cur_site + 1, adj_site)));
        }
      }
  ...
  for (int i = 0; i < _num_sites; ++i) {
    ...
    do {
      ...
      if (from_sym_to_id.find(_p) != from_sym_to_id.end()) {
        ...
        _cells[i].push_back(
            std::make_pair(_tessellation_vertices.size(), _adj));
