...
_ICP::_ICP(const std::vector<_Point3> &static_points)
  ...
  _max_iteration = 400;
  _num_random_sample = 400 > (int)(_static_points.size()) / 5
                           ? 400
                           : (int)(_static_points.size());
  _epsilon = 1e-8;
}
Eigen::Matrix4d _ICP::registration_(std::vector<_Point3> dynamic_points) const {
  ...
  while (1) {
    ...
    if (cost < _epsilon || iter == _max_iteration)
      break;
    Eigen::JacobiSVD<Eigen::Matrix3d> svd(U, Eigen::ComputeFullU |
                                                 Eigen::ComputeFullV);
...
Eigen::Matrix4d _ICP::registration_(std::vector<_Point3> dynamic_points,
  ...
  while (1) {
    ...
    if (cost < _epsilon || iter == _max_iteration)
      break;
    Eigen::JacobiSVD<Eigen::Matrix3d> svd(U, Eigen::ComputeThinU |
                                                 Eigen::ComputeThinV);
    Eigen::Matrix3d svdU = svd.matrixU();
    Eigen::Matrix3d svdV = svd.matrixV().transpose();
    rotationM = svdU * svdV;
