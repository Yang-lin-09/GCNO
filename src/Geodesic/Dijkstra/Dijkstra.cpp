...
namespace BGAL {
namespace Geodesic {
void _Dijkstra::initialize_() { _Abstract_Method::initialize_(); }
void _Dijkstra::implement_() {
  struct _Event : std::tuple<double, int, int, int, int> {
    _Event(double dis, int self, int parent, int root, int level)
        : std::tuple<double, int, int, int, int>(dis, self, parent, root,
                                                 level) {}
    ...
    double get_distance_() const { return std::get<0>(*this); }
    int get_id_() const { return std::get<1>(*this); }
    int get_parent_() const { return std::get<2>(*this); }
    int get_root_() const { return std::get<3>(*this); }
    int get_level_() const { return std::get<4>(*this); }
  };
  ...
  while (!evtQue.empty()) {
    ...
    _result[topEvt.get_id_()] =
        std::make_tuple(topEvt.get_parent_(), topEvt.get_root_(),
                        topEvt.get_level_(), topEvt.get_distance_());
    for (auto veit = _model.ve_begin(topEvt.get_id_());
         veit != _model.ve_end(topEvt.get_id_()); ++veit) {
      int v = (*veit)._id_right_vertex;
      if (visited[v])
        continue;
      _Event evt(topEvt.get_distance_() + (*veit).length_(), v,
                 topEvt.get_id_(), topEvt.get_root_(), topEvt.get_level_() + 1);
      evtQue.push(evt);
    }
  }
...
_Dijkstra::_Dijkstra(const _ManifoldModel &in_model,
                     const std::map<int, double> &in_sources)
