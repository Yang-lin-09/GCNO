...
_Tessellation3D_Skeleton::_Tessellation3D_Skeleton(Rt &rt,
                                                   const int &num_vertices) {
  _neights.resize(num_vertices);
  for (auto c_it = rt.finite_cells_begin(); c_it != rt.finite_cells_end();
       c_it++) {
    int i0, i1, i2, i3;
...
_BOC::_Sign _Restricted_Tessellation3D::side_(const int &ip1, const int &ip2,
                                              _Symbolic_Point &v) {
  ...
  case 1: {
    ...
    res = _Side3D::side1_(p1.x(), p1.y(), p1.z(), p2.x(), p2.y(), p2.z(),
                          _weights[ip1], _weights[ip2], q.x(), q.y(), q.z());
    break;
  }
  case 2: {
    ...
    res = _Side3D::side2_(p1.x(), p1.y(), p1.z(), p2.x(), p2.y(), p2.z(),
                          p3.x(), p3.y(), p3.z(), _weights[ip1], _weights[ip2],
                          _weights[v.p[2]], q1.x(), q1.y(), q1.z(), q2.x(),
                          q2.y(), q2.z());
    break;
  }
  case 3: {
    ...
    res = _Side3D::side3_(p1.x(), p1.y(), p1.z(), p2.x(), p2.y(), p2.z(),
                          p3.x(), p3.y(), p3.z(), p4.x(), p4.y(), p4.z(),
                          _weights[ip1], _weights[ip2], _weights[ip3],
                          _weights[ip4], q1.x(), q1.y(), q1.z(), q2.x(), q2.y(),
                          q2.z(), q3.x(), q3.y(), q3.z());
    break;
  }
  default:
...
_Restricted_Tessellation3D::insec_bisector_(
    const _Symbolic_Point &p1, const _Symbolic_Point &p2, const int &neigh,
    const int &center, const std::vector<int> &current_face_vid) const {
...
void _Restricted_Tessellation3D::calculate_() {
  ...
  for (int i = 0; i < _num_sites; ++i) {
    wps[i] = std::make_pair(
        Weighted_point(Point(_sites[i].x(), _sites[i].y(), _sites[i].z()),
                       _weights[i]),
        i);
  }
  ...
  wps.push_back(
      std::make_pair(Weighted_point(Point(3 * min_p.x() - 2 * max_p.x(),
                                          3 * min_p.y() - 2 * max_p.y(),
                                          3 * min_p.z() - 2 * max_p.z()),
                                    min_weight),
                     -1));
  wps.push_back(
      std::make_pair(Weighted_point(Point(3 * max_p.x() - 2 * min_p.x(),
                                          3 * min_p.y() - 2 * max_p.y(),
                                          3 * min_p.z() - 2 * max_p.z()),
                                    min_weight),
                     -1));
  ...
  for (_Face_Iterator f_it = _model.face_begin(); f_it != _model.face_end();
       ++f_it) {
      ...
      continue;
    {
      ...
      for (auto fe_it = _model.fe_begin(f_it.id());
           fe_it != _model.fe_end(f_it.id()); ++fe_it) {
      ...
      fcenter = fcenter / 3.0;
      int near_site = rt.nearest_power_vertex(
                            K::Point_3(fcenter.x(), fcenter.y(), fcenter.z()))
                          ->info();
      std::vector<_Symbolic_Point> fps;
      _Symbolic_Point fp1(-f_idx[0] - 1, -f_idx[1] - 1, -f_idx[3] - 1,
                          near_site);
      fp1.flag = 1;
      fp1.p.push_back(v_idx[0]);
      _Symbolic_Point fp2(-f_idx[0] - 1, -f_idx[1] - 1, -f_idx[2] - 1,
                          near_site);
      fp2.flag = 1;
      fp2.p.push_back(v_idx[1]);
      _Symbolic_Point fp3(-f_idx[0] - 1, -f_idx[2] - 1, -f_idx[3] - 1,
                          near_site);
      fp3.flag = 1;
    ...
    while (!Qfs.empty()) {
      ...
      const std::vector<_Symbolic_Point> original_cliped =
          cliped_faces[current_cliped];
      ...
      for (auto p = planes.begin(); p != planes.end(); ++p) {
        ...
        // for (auto tit = old_cliped[0]._sym.begin(); tit !=
        // old_cliped[0]._sym.end(); ++tit)
        //{
        //   //std::cout << *tit << " ";
        // }
        ...
        if (pre_state == _BOC::_Sign::PositivE) {
          ...
          if (cur_state == _BOC::_Sign::PositivE) {
            ...
            update_cliped.push_back(
                insec_bisector_(old_cliped[0], old_cliped[1], *p, current_site,
                                current_face_vid));
        ...
        for (int i = 2; i < old_cliped.size(); ++i) {
          ...
          if (cur_state == _BOC::_Sign::PositivE) {
            if (nex_state == _BOC::_Sign::PositivE) {
              ...
              update_cliped.push_back(
                  insec_bisector_(old_cliped[i - 1], old_cliped[i], *p,
                                  current_site, current_face_vid));
            } else {
        ...
        if (sp0 == _BOC::_Sign::PositivE) {
          if (cur_state == _BOC::_Sign::NegativE) {
            update_cliped.push_back(insec_bisector_(
                old_cliped[old_cliped.size() - 1], old_cliped[0], *p,
                current_site, current_face_vid));
          } else if (cur_state == _BOC::_Sign::ZerO) {
            if (pre_state == _BOC::_Sign::NegativE) {
      ...
      for (int i = 0; i < old_cliped.size(); ++i) {
        for (auto it = old_cliped[i]._sym.begin();
             it != old_cliped[i]._sym.end(); ++it) {
          if (*it > 0 && from_idx_to_locations[(*it) - 1].find(current_face) ==
                             from_idx_to_locations[(*it) - 1].end()) {
            from_idx_to_locations[(*it) - 1][current_face] =
                cliped_faces.size();
          ...
          if (*it < 0 && from_idx_to_locations[current_site].find(-(*it) - 1) ==
                             from_idx_to_locations[current_site].end()) {
            std::vector<int> v_idx;
            std::vector<int> f_idx;
            f_idx.push_back(-(*it) - 1);
            for (auto afe_it = _model.fe_begin(-(*it) - 1);
                 afe_it != _model.fe_end(-(*it) - 1); ++afe_it) {
              int cv = (*afe_it)._id_left_vertex;
              v_idx.push_back(cv);
              f_idx.push_back(
                  _model.edge_((*afe_it)._id_reverse_edge)._id_face);
            }
            std::vector<_Symbolic_Point> fps;
            _Symbolic_Point fp1(-f_idx[0] - 1, -f_idx[1] - 1, -f_idx[3] - 1,
                                current_site);
            ...
            fps.push_back(fp2);
            fps.push_back(fp3);
            from_idx_to_locations[current_site][-(*it) - 1] =
                cliped_faces.size();
  ...
  for (int i = 0; i < _num_sites; ++i) {
    for (auto it = from_idx_to_locations[i].begin();
         it != from_idx_to_locations[i].end(); ++it) {
      ...
      if (cliped_faces[it->second][0].flag == 1) {
        ...
        if (from_Sym_to_vertex.find(psym) == from_Sym_to_vertex.end()) {
          ...
          double d = _weights[q2] - _weights[q1] -
                     0.5 * v.dot_(_sites[q2] + _sites[q1]);
        ...
        std::vector<int> p12;
        int q = -1;
        for (auto tit = cliped_faces[it->second][0]._sym.begin();
             tit != cliped_faces[it->second][0]._sym.end(); ++tit) {
          if (*tit > 0) {
            psym.insert(*tit);
            p12.push_back(*tit - 1);
          } else
        ...
        if (from_Sym_to_vertex.find(psym) == from_Sym_to_vertex.end()) {
          ...
          double d1 = _weights[p12[0]] - _weights[i] -
                      0.5 * v1.dot_(_sites[p12[0]] + _sites[i]);
          _Point3 v2 = (_sites[p12[1]] - _sites[i]) * 2;
          double d2 = _weights[p12[1]] - _weights[i] -
                      0.5 * v2.dot_(_sites[p12[1]] + _sites[i]);
          _Point3 v0 = _model.normal_face_(q);
          v0.normalized_();
          double d0 =
              -v0.dot_(_model.vertex_(cliped_faces[it->second][0].p[0]));
          _Point3 cp3 =
              _Point3::intersection_three_plane(v0, d0, v1, d1, v2, d2);
          from_Sym_to_vertex[psym] = _vertices.size();
          firest_p = _vertices.size();
          _vertices.push_back(cp3);
        } else {
      ...
      if (cliped_faces[it->second][1].flag == 1) {
        ...
        std::vector<int> p12;
        int q = -1;
        for (auto tit = cliped_faces[it->second][1]._sym.begin();
             tit != cliped_faces[it->second][1]._sym.end(); ++tit) {
          if (*tit > 0) {
            psym.insert(*tit);
            p12.push_back(*tit - 1);
          } else
        ...
        if (from_Sym_to_vertex.find(psym) == from_Sym_to_vertex.end()) {
          ...
          v0.normalized_();
          double d0 =
              -v0.dot_(_model.vertex_(cliped_faces[it->second][1].p[0]));
          _Point3 cp3 =
              _Point3::intersection_three_plane(v0, d0, v1, d1, v2, d2);
          from_Sym_to_vertex[psym] = _vertices.size();
          second_p = _vertices.size();
      ...
      {
        std::set<int> insec_sym =
            cliped_faces[it->second][0].insec_(cliped_faces[it->second][1]);
        int adj_sites = -1;
      ...
      for (int j = 2; j < cliped_faces[it->second].size(); ++j) {
        ...
        if (cliped_faces[it->second][j].flag == 1) {
          ...
          if (from_Sym_to_vertex.find(psym) == from_Sym_to_vertex.end()) {
            ...
            _vertices.push_back(
                _model.vertex_(cliped_faces[it->second][j].p[0]));
          } else {
          ...
          std::vector<int> p12;
          int q = -1;
          for (auto tit = cliped_faces[it->second][j]._sym.begin();
               tit != cliped_faces[it->second][j]._sym.end(); ++tit) {
            if (*tit > 0) {
              psym.insert(*tit);
              p12.push_back(*tit - 1);
            } else
          ...
          if (from_Sym_to_vertex.find(psym) == from_Sym_to_vertex.end()) {
            ...
            v0.normalized_();
            double d0 =
                -v0.dot_(_model.vertex_(cliped_faces[it->second][j].p[0]));
            _Point3 cp3 =
                _Point3::intersection_three_plane(v0, d0, v1, d1, v2, d2);
            from_Sym_to_vertex[psym] = _vertices.size();
            third_p = _vertices.size();
        ...
        {
          std::set<int> insec_sym = cliped_faces[it->second][j - 1].insec_(
              cliped_faces[it->second][j]);
          int adj_sites = -1;
          for (auto tit = insec_sym.begin(); tit != insec_sym.end(); ++tit) {
            if (*tit > 0)
              adj_sites = *tit - 1;
      ...
      {
        std::set<int> insec_sym =
            cliped_faces[it->second][0].insec_(cliped_faces[it->second].back());
        int adj_sites = -1;
        for (auto tit = insec_sym.begin(); tit != insec_sym.end(); ++tit) {
          if (*tit > 0)
            adj_sites = *tit - 1;
...
_Restricted_Tessellation3D::_Restricted_Tessellation3D(
    const _ManifoldModel &in_model)
    : _model(in_model) {
  _num_sites = 0;
  _sites.clear();
  _weights.clear();
...
_Restricted_Tessellation3D::_Restricted_Tessellation3D(
    const _ManifoldModel &in_model, const std::vector<_Point3> &in_sites,
    const std::vector<double> &in_weights)
    : _num_sites(in_sites.size()), _sites(in_sites), _weights(in_weights),
      _model(in_model) {
  _vertices.clear();
  _cells.clear();
  _cells.resize(_num_sites);
...
_Restricted_Tessellation3D::_Restricted_Tessellation3D(
    const _ManifoldModel &in_model, const std::vector<_Point3> &in_sites)
    : _num_sites(in_sites.size()), _sites(in_sites), _model(in_model) {
  _vertices.clear();
  _cells.clear();
  _cells.resize(_num_sites);
  _weights.clear();
