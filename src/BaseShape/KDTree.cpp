...
void _KDTree::build_(const std::vector<_Point3> &in_points) {
  ...
  std::nth_element(
      ids.data(), ids.data() + mid, ids.data() + (int)(ids.size()),
      [&](int lhs, int rhs) { return _points[lhs][0] < _points[rhs][0]; });
  _root->_id = ids[mid];
  S.push(std::make_tuple(ids.data(), mid, 1, _root, 0));
  S.push(std::make_tuple(ids.data() + mid + 1, (int)(ids.size()) - mid - 1, 1,
                         _root, 1));
  while (!S.empty()) {
    if (std::get<1>(S.top()) <= 0) {
      ...
      S.push(std::make_tuple(sid + mid + 1, std::get<1>(state) - mid - 1,
                             std::get<2>(state) + 1, _node, 1));
    }
  }
}
void _KDTree::clear_() {
