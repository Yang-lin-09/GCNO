...
_Polygon::_Polygon(const std::vector<_Point2> &in_points) {
  ...
  _bounding_box =
      std::make_pair(_Point2(__minx, __miny), _Point2(__maxx, __maxy));
}
...
bool _Polygon::is_in_(const _Point2 &in_p) const {
  ...
  for (int i = 0; i < _points.size(); ++i) {
    ...
    if (_BOC::sign_((__p1 - in_p).cross_(__p2 - in_p).length_()) ==
            _BOC::_Sign::ZerO &&
        (_BOC::sign_((__p1 - in_p).dot_((__p2 - in_p))) ==
             _BOC::_Sign::NegativE ||
         _BOC::sign_((__p1 - in_p).dot_((__p2 - in_p))) == _BOC::_Sign::ZerO)) {
      return false;
    }
    if (((_BOC::sign_(__p1.y() - in_p.y()) == _BOC::_Sign::PositivE) !=
         (_BOC::sign_(__p2.y() - in_p.y()) == _BOC::_Sign::PositivE)) &&
        _BOC::sign_(in_p.x() -
                    (in_p.y() - __p1.y()) * (__p1.x() - __p2.x()) /
                        (__p1.y() - __p2.y()) -
                    __p1.x()) == _BOC::_Sign::NegativE) {
      __res = !__res;
    }
  }
  return __res;
...
_Point2 _Polygon::nearest_point_(const _Point2 &in_p) {
  ...
  for (int i = 0; i < num_(); ++i) {
    if (_BOC::sign_((_points[(i + 1) % num_()] - _points[i]).length_()) ==
        _BOC::_Sign::ZerO) {
    ...
    double len =
        (_points[(i + 1) % num_()] - _points[i]).dot_(in_p - _points[i]);
    _Point2 lp;
    if (len < 0 || len > (_points[(i + 1) % num_()] - _points[i]).sqlength_()) {
      ...
      lp = _points[i] +
           (_points[(i + 1) % num_()] - _points[i]) * len /
               ((_points[(i + 1) % num_()] - _points[i]).sqlength_());
      len = (lp - in_p).length_();
    }
    if (len < min_dis) {
      min_dis = len;
...
int _Polygon::intersection_with_linesegment_(
    const _Point2 &p1, const _Point2 &p2,
    std::vector<std::pair<int, _Point2>> &intersections) const {
  bool add_p1 = false;
  bool add_p2 = false;
  int p1_inx = -1;
  int p2_inx = -1;
  ...
  for (int i = 0; i < _points.size(); ++i) {
    ...
    if (_BOC::sign_(
            (_points[i] - p1).dot_(_points[(i + 1) % _points.size()] - p1) +
            ((_points[i] - p1).length_()) *
                ((_points[(i + 1) % _points.size()] - p1).length_())) ==
        _BOC::_Sign::ZerO) {
      p1_on = true;
    }
    if (_BOC::sign_(
            ((_points[i] - p2).dot_(_points[(i + 1) % _points.size()] - p2) +
             ((_points[i] - p2).length_()) *
                 ((_points[(i + 1) % _points.size()] - p2).length_()))) ==
        _BOC::_Sign::ZerO) {
      p2_on = true;
    }
    ...
    if (_BOC::sign_((p1 - p2)
                        .cross_(_points[i] - _points[(i + 1) % _points.size()])
                        .length_()) == _BOC::_Sign::ZerO) {
      if (p1_on && (!p2_on)) {
        intersections.resize(0);
      ...
      if (p2_on && (!p1_on)) {
        ...
        if (p2 == _points[i]) {
          intersections.push_back(
              std::make_pair((i + _points.size() - 1) % _points.size(), p2));
        } else {
          intersections.push_back(std::make_pair(i, p2));
        }
        return 1;
    ...
    if (_BOC::sign_((p1 - _points[i]).dot_(p2 - _points[i]) +
                    ((p1 - _points[i]).length_()) *
                        ((p2 - _points[i]).length_())) == _BOC::_Sign::ZerO) {
      intersections.push_back(std::make_pair(i, _points[i]));
      is_on_vertex.push_back(true);
      continue;
    }
    if (_BOC::sign_((p1 - _points[(i + 1) % _points.size()])
                        .dot_(p2 - _points[(i + 1) % _points.size()]) +
                    ((p1 - _points[(i + 1) % _points.size()]).length_()) *
                        ((p2 - _points[(i + 1) % _points.size()]).length_())) ==
        _BOC::_Sign::ZerO) {
      continue;
    }

    double __c1 = (_points[i] - _points[(i + 1) % _points.size()])
                      .cross_(_points[i] - p1)
                      .z();
    double __c2 = (_points[i] - _points[(i + 1) % _points.size()])
                      .cross_(_points[i] - p2)
                      .z();
  ...
  if (intersections.size() == 2) {
    ...
    if (is_on_vertex[0]) {
      intersections[0].first =
          (intersections[0].first + _points.size() - 1) % _points.size();
    }
    return 2;
  }
