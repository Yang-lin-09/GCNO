...
namespace BGAL {
_Model::_MFace::_MFace() : _Triangle3(), id(-1) { _vertices.resize(3, -1); }
_Model::_MFace::_MFace(const int &id1, const int &id2, const int &id3)
    : _Triangle3(), id(-1) {
  _vertices.resize(3);
  _vertices[0] = id1;
  _vertices[1] = id2;
...
_Model::_MFace::_MFace(const int &id1, const int &id2, const int &id3,
                       const _Point3 &p1, const _Point3 &p2, const _Point3 &p3)
...
int &_Model::_MFace::operator[](int index) { return _vertices[index]; }
int _Model::_MFace::operator[](int index) const { return _vertices[index]; }
...
_Model::_Model(const std::string &in_file_name) {
  int folder_loc = in_file_name.rfind("\\") > in_file_name.rfind("/")
                       ? in_file_name.rfind("\\")
                       : in_file_name.rfind("/");
...
_Face_Iterator _Model::face_begin() const { return _Face_Iterator(this, 0); }
...
void _Model::save_obj_file_(const std::string &in_file_name) const {
  ...
  int folder_loc = in_file_name.rfind("\\") > in_file_name.rfind("/")
                       ...
                       : in_file_name.rfind("/");
  int dot_loc = in_file_name.rfind('.');
  out << "g " << in_file_name.substr(folder_loc + 1, dot_loc - folder_loc - 1)
      << std::endl;
  for (int i = 0; i < _vertices.size(); ++i) {
    out << "v " << std::setiosflags(std::ios::fixed) << std::setprecision(20)
  ...
  for (int i = 0; i < _faces.size(); ++i) {
    ...
    out << "f " << _faces[i][0] + 1 << " " << _faces[i][1] + 1 << " "
        << _faces[i][2] + 1 << std::endl;
  }
  out.close();
}
void _Model::save_scalar_field_obj_file_(
    const std::string &in_file_name, const std::vector<double> &vals) const {
  std::ofstream out(in_file_name);
  int folder_loc = in_file_name.rfind("\\") > in_file_name.rfind("/")
                       ? in_file_name.rfind("\\")
                       : in_file_name.rfind("/");
  ...
  out << "g " << in_file_name.substr(folder_loc + 1, dot_loc - folder_loc - 1)
      << std::endl;
  out << "# max value: " << *std::max_element(vals.begin(), vals.end())
      << std::endl;
  ...
  for (int i = 0; i < vals.size(); ++i) {
    out << "vt " << std::setiosflags(std::ios::fixed) << std::setprecision(20)
        << vals[i] << " 0" << std::endl;
  }
...
void _Model::save_pamametrization_obj_file_(
  ...
  for (int i = 0; i < uvs.size(); ++i) {
    out << "vt " << std::setiosflags(std::ios::fixed) << std::setprecision(20)
        << uvs[i].first << " " << uvs[i].second << std::endl;
  }
  for (int i = 0; i < number_faces_(); ++i) {
    if (_faces_useless.find(i) != _faces_useless.end()) {
      continue;
...
std::tuple<_Point3, double, int>
_Model::nearest_point_(const _Point3 &in_point) {
  _PQP_Query_Resutl query_res = proximity_query_(in_point);
  return std::make_tuple(query_res._nearest_point, query_res._distance,
                         query_res._triangle_id);
}
double _Model::signed_distance_(const _Point3 &in_point) {
  _PQP_Query_Resutl query_res = proximity_query_(in_point);
  double dis1 =
      (query_res._nearest_point - vertex_(face_(query_res._triangle_id)[0]))
          .length_();
  double dis2 =
      (query_res._nearest_point - vertex_(face_(query_res._triangle_id)[1]))
          .length_();
  double dis3 =
      (query_res._nearest_point - vertex_(face_(query_res._triangle_id)[2]))
          .length_();
...
void _Model::read_obj_file_(const std::string &in_file_name) {
  ...
  while (std::getline(in, line)) {
    ...
    if (word == "v") {
      ...
      sline >> id0 >> id1;
      while (sline >> idx) {
        _Model::_MFace f(id0 - 1, id1 - 1, idx - 1, _vertices[id0 - 1],
                         _vertices[id1 - 1], _vertices[idx - 1]);
        f.id = _faces.size();
        _faces.push_back(f);
        id1 = idx;
      }
...
void _Model::compute_normal_boundingbox_() {
  ...
  for (int i = 0; i < _faces.size(); ++i) {
    _Point3 norm =
        (_vertices[_faces[i][1]] - _vertices[_faces[i][0]])
            .cross_((_vertices[_faces[i][2]] - _vertices[_faces[i][0]]));
...
_Model::_PQP_Query_Resutl::_PQP_Query_Resutl(const int &in_pos_flag,
    ...
    : _pos_flag(in_pos_flag), _triangle_id(in_triangle_id),
      _distance(in_distance), _nearest_point(in_nearset_point) {}
} // namespace BGAL
