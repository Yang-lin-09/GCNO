...
_ManifoldModel::_MMEdge::_MMEdge()
    : _Segment3(), _id_left_vertex(-1), _id_right_vertex(-1),
      _id_opposite_vertex(-1), _id_left_edge(-1), _id_right_edge(-1),
      _id_reverse_edge(-1), _id_face(-1) {}
_ManifoldModel::_MMEdge::_MMEdge(const _Point3 &in_s, const _Point3 &in_t)
    : _Segment3(in_s, in_t), _id_left_vertex(-1), _id_right_vertex(-1),
      _id_opposite_vertex(-1), _id_left_edge(-1), _id_right_edge(-1),
      _id_reverse_edge(-1), _id_face(-1) {}
_ManifoldModel::_ManifoldModel() {}
_ManifoldModel::_ManifoldModel(const std::string &in_file_name)
    : _Model(in_file_name) {
  preprocess_model_();
}
_ManifoldModel::_ManifoldModel(const std::vector<_Point3> &in_vertices,
  ...
  for (int i = 0; i < _faces.size(); ++i) {
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      std::map<std::pair<int, int>, int>::const_iterator it =
          from_idpair_to_location.find(
              std::make_pair(left_vertex, right_vertex));
      if (it != from_idpair_to_location.end()) {
        ...
        _edges.push_back(e1);
        from_idpair_to_location[std::make_pair(left_vertex, right_vertex)] =
        ...
        _edges.push_back(e2);
        from_idpair_to_location[std::make_pair(right_vertex, left_vertex)] =
            _edges.size() - 1;
      }
    }
    for (int j = 0; j < 3; ++j) {
      _edges[eid_in_f[j]]._id_left_edge =
          _edges[eid_in_f[(j + 2) % 3]]._id_reverse_edge;
      _edges[eid_in_f[j]]._id_right_edge =
          _edges[eid_in_f[(j + 1) % 3]]._id_reverse_edge;
    }
  }
}
void _ManifoldModel::arrange_neighs_of_vertex_face_() {
  ...
  for (int i = 0; i < _edges.size(); ++i) {
    if (_edges[i]._id_face == -1)
      continue;
    if (_neight_edge_of_vertices[_edges[i]._id_left_vertex] == -1 ||
        _edges[_edges[i]._id_reverse_edge]._id_face == -1) {
