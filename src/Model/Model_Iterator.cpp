...
_FV_Iterator::_FV_Iterator(const _Model *in_model, const int &in_fid,
                           const int &in_cursor)
...
_VF_Iterator::_VF_Iterator(const _ManifoldModel *in_model, const int &in_vid,
                           const int &in_cursor)
...
const _Model::_MFace &_VF_Iterator::operator*() { return _model->_faces[id()]; }
...
_Edge_Iterator::_Edge_Iterator(const _ManifoldModel *in_model,
                               const int &in_cursor)
    : _model(in_model), _cursor(in_cursor) {}
...
_FE_Iterator::_FE_Iterator(const _ManifoldModel *in_model, const int &in_fid,
                           const int &in_cursor)
    : _model(in_model), _fid(in_fid), _cursor(in_cursor) {
  if (_cursor != 0) {
...
_FF_Iterator::_FF_Iterator(const _ManifoldModel *in_model, const int &in_fid,
                           const int &in_cursor)
    : _model(in_model), _fid(in_fid), _cursor(in_cursor) {
  if (_cursor != 0) {
    throw std::runtime_error("index error! cursor=" + std::to_string(_cursor));
...
_VV_Iterator::_VV_Iterator(const _ManifoldModel *in_model, const int &in_vid,
                           const int &in_cursor)
    : _model(in_model), _vid(in_vid), _cursor(in_cursor) {
  if (_cursor != 0) {
    throw std::runtime_error("index error! cursor=" + std::to_string(_cursor));
...
const _Point3 &_VV_Iterator::operator*() { return _model->_vertices[id()]; }
_VV_Iterator &_VV_Iterator::operator++() {
  if (_model
          ->edge_(_model->edge_(_model->_neight_edge_of_vertices[_vid])
                      ._id_reverse_edge)
          ._id_face == -1) {
...
_VE_Iterator::_VE_Iterator(const _ManifoldModel *in_model, const int &in_vid,
                           const int &in_cursor)
    : _model(in_model), _vid(in_vid), _cursor(in_cursor) {
  if (_cursor != 0) {
    throw std::runtime_error("index error! cursor=" + std::to_string(_cursor));
