...
public:
  ...
  const std::set<int> &neight_(const int &in_i) const { return _neights[in_i]; }
};
...
private:
  struct _Symbolic_Point {
    ...
    _Symbolic_Point(const int &in1, const int &in2, const int &in3,
                    const int &in_site)
    ...
    void insert_(const int &in) { _sym.insert(in); }
  ...
  _Symbolic_Point
  insec_bisector_(const _Symbolic_Point &p1, const _Symbolic_Point &p2,
                  const int &neigh, const int &center,
...
public:
  ...
  int number_vertices_() const { return _vertices.size(); }
  ...
  const std::vector<_Point3> &get_sites_() const { return _sites; }
  const std::vector<std::vector<std::tuple<int, int, int>>> &
  get_cells_() const {
    return _cells;
  }
