...
public:
  ...
  static void mark_domain(CDT &cdt) {
    std::function<void(CDT & ct, CGAL_Face_handle start, int index,
                       std::list<CDT::Edge> &boreder)>
        mark_domains = [](CDT &ct, CGAL_Face_handle start, int index,
  ...
  void end_();
  inline int num_() const { return _points.size(); }
  ...
  int intersection_with_linesegment_(
  ...
  std::vector<_Polygon> constrained_delaunay_triangulation_() const {
    ...
    for (CDT::Finite_faces_iterator fit = cdt.finite_faces_begin();
      ...
      if (fit->info().in_domain()) {
        ...
        tri.start_();
        tri.insert_(
            _Point2(fit->vertex(0)->point().x(), fit->vertex(0)->point().y()));
        tri.insert_(
            _Point2(fit->vertex(1)->point().x(), fit->vertex(1)->point().y()));
        tri.insert_(
            _Point2(fit->vertex(2)->point().x(), fit->vertex(2)->point().y()));
        tri.end_();
