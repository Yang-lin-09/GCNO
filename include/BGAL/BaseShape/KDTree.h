...
public:
  ...
  std::vector<int> nsearch_(const std::vector<_Point3> &in_ps,
                            const int &k) const;
  std::vector<int> rsearch_(const std::vector<_Point3> &in_p,
                            const double &in_r) const;
...
private:
  ...
  struct _Node {
    ...
    _Node *_next[2];
    int _axis;
    _Node() : _id(-1), _axis(-1) { _next[0] = _next[1] = nullptr; }
  };
