...
private:
  template <class fun>
  int linear_search_(fun &f, double &fval, Eigen::VectorXd &iterX,
                     Eigen::VectorXd &gradient, double &step,
...
template <class fun> int _LBFGS::minimize(fun &f, Eigen::VectorXd &iterX) {
  ...
  while (1) {
    ...
    if (_parameter.max_time > 0 &&
        _parameter.max_time < (clock() - start_t) * 1000.0 / CLOCKS_PER_SEC) {
      if (_parameter.is_show) {
...
template <class fun>
int _LBFGS::linear_search_(fun &f, double &fval, Eigen::VectorXd &iterX,
                           Eigen::VectorXd &gradient, double &step,
                           const Eigen::VectorXd &direction) {
  if (step < 0) {
