...
public:
  ...
  inline int edge_end() const { return number_edges_(); }
  ...
  inline int vv_end(const int &vid) const {
    ...
    if (edge_(edge_(_neight_edge_of_vertices[vid])._id_reverse_edge)._id_face ==
        -1) {
  ...
  inline bool is_boundary_vertex_(const int &vid) const {
    ...
    if (_neight_edge_of_vertices[vid] != -1 &&
        _edges[_edges[_neight_edge_of_vertices[vid]]._id_reverse_edge]
                ._id_face == -1)
      return true;
    else
      return false;
