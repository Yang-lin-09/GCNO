...
public:
  ...
  public:
    ...
    _MFace(const int &id1, const int &id2, const int &id3, const _Point3 &p1,
           const _Point3 &p2, const _Point3 &p3);
  ...
  public:
    ...
    _PQP_Query_Resutl();
    _PQP_Query_Resutl(const int &in_pos_flag, const int &in_triangle_id,
                      const double &in_distance,
                      const _Point3 &in_nearset_point);
  };
  _Model();
  _Model(const std::string &in_file_name);
  void set_name_(const std::string &in_name) { _name = in_name; }
  inline int number_vertices_() const { return _vertices.size(); }
  inline int number_faces_() const { return _faces.size(); }
  ...
  inline int face_end() const { return number_faces_(); }
  _Vertex_Iterator vertex_begin() const;
  inline int vertex_end() const { return number_vertices_(); }
  ...
  void save_scalar_field_obj_file_(const std::string &in_file_name,
                                   const std::vector<double> &vals) const;
  void save_pamametrization_obj_file_(
      const std::string &in_file_name,
      const std::vector<std::pair<double, double>> &uvs) const;
  inline std::pair<_Point3, _Point3> bounding_box_() const {
    return _bounding_box;
  }
  inline std::string name_() const { return _name; }
