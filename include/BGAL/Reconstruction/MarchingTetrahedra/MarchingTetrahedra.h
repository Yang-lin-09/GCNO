...
public:
  ...
  _Marching_Tetrahedra(const std::pair<_Point3, _Point3> &in_boundingbox,
                       const int &in_depth);
  template <class F> _ManifoldModel reconstruction_(const F &sign);
  void set_method_(const int &m) { _method = m; }
...
_ManifoldModel _Marching_Tetrahedra::reconstruction_(const F &sign) {
  ...
  for (int i = 0; i < _tetras.size(); ++i) {
    ...
    if (negetive.size() == 1) {
      ...
      for (int j = 0; j < 3; ++j) {
        ...
        if (from_edge_to_vertex.find(std::make_pair(v0, v1)) ==
            from_edge_to_vertex.end()) {
          from_edge_to_vertex[std::make_pair(v0, v1)] = _vertices.size();
          switch (_method) {
          case 0: {
            _vertices.push_back(
                sign((_tetra_vertices[v0] + _tetra_vertices[v1]) * 0.5).second);
            break;
          }
          case 1: {
            double len = fabs(signs[v0]) + fabs(signs[v1]);
            _vertices.push_back(_tetra_vertices[v0] * fabs(signs[v1]) / len +
                                _tetra_vertices[v1] * fabs(signs[v0] / len));
            break;
          }
          default:
            break;
      ...
      if (negetive[0] == std::get<3>(_tetras[i]) ||
          negetive[0] == std::get<1>(_tetras[i])) {
      ...
      if (positive[0] == std::get<3>(_tetras[i]) ||
          positive[0] == std::get<1>(_tetras[i])) {
        tris.push_back(std::make_tuple(tri_v[0], tri_v[1], tri_v[2]));
      } else {
        tris.push_back(std::make_tuple(tri_v[2], tri_v[1], tri_v[0]));
      }
  ...
  for (int i = 0; i < tris.size(); ++i) {
    faces.push_back(_Model::_MFace(std::get<0>(tris[i]), std::get<1>(tris[i]),
                                   std::get<2>(tris[i])));
  }
