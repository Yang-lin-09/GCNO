...
struct MyFaceCVT {
  MyFaceCVT(Eigen::Vector3i a) { p = a; }
...
public:
  ...
  _CVT3D(const _ManifoldModel &model, std::function<double(_Point3 &p)> &rho,
         _LBFGS::_Parameter para);
  _CVT3D(const _ManifoldModel &model, std::function<double(_Point3 &p)> &rho,
         _LBFGS::_Parameter para, std::string modelname);
